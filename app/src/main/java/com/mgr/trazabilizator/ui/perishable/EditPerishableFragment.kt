...
class EditPerishableFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val root = inflater.inflate(R.layout.fragment_editperishable, container, false)
        return root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        ID = clipData
        if (ID.contains("ID:")) {
            db.collection("perishable").document(ID.drop(3)).get()
                .addOnSuccessListener { document ->
                    ...
                    view.findViewById<TextInputEditText>(R.id.TextInputEditTextTemperature)
                        .setText(document?.data?.get("Temperature").toString())
                    view.findViewById<TextInputEditText>(R.id.TextInputEditTextCondition)
                        .setText(document?.data?.get("Condition").toString())
                    view.findViewById<TextInputEditText>(R.id.TextInputEditTextExpiryDate)
                        .setText(document?.data?.get("ExpiryDate").toString())
        ...
        view.findViewById<Button>(R.id.buttonUpdate).setOnClickListener() {
            ...
            temperature =
                view.findViewById<TextInputEditText>(R.id.TextInputEditTextTemperature).text.toString()
            condition =
                view.findViewById<TextInputEditText>(R.id.TextInputEditTextCondition).text.toString()
            expiryDate =
                view.findViewById<TextInputEditText>(R.id.TextInputEditTextExpiryDate).text.toString()
            startDate =
            ...
            if (checkEmpty(date, product, amount, responsable, condition)) {
                try {
                    db.collection("perishable").document(ID.drop(3)).update(
                        mapOf(
                            "Date" to date,
                            "Product" to product,
                    ...
                    Toast.makeText(context, R.string.successfullyUpdate, Toast.LENGTH_LONG)
                        .show()
